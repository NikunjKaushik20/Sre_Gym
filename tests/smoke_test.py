...
This module avoids exiting at import time so pytest collection does not fail.
"""
import json
import sys
...
def run_smoke_test() -> int:
    ...
    chk("substring rejected", not _fuzzy("memory_leak","memory_leak_issue"))
    chk("rejects unrelated", not _fuzzy("traffic_spike","memory_leak"))

    # ── Graders ──
    print("\n=== GRADERS ===")
    s = json.loads((SCENARIOS_DIR/"easy_redis_oom.json").read_text())
    easy_perfect = grade_easy(1.0, st(diagnosed=True,diagnosed_service="redis", diagnosed_cause="memory_leak",
        remediated=True,remediation_applied="restart_redis"), s)
    chk("easy perfect (health=1.0)", 0.70 <= easy_perfect < 1.0, f"score={easy_perfect}")
    chk("easy zero health", grade_easy(0.0, st(), s) == 0.01)
    chk("easy partial health", 0.01 <= grade_easy(0.5, st(diagnosed=True,diagnosed_service="redis"), s) < 1.0)

    s = json.loads((SCENARIOS_DIR/"medium_cascade_redis.json").read_text())
    chk("medium no-op=0.01", grade_medium(0.0, st(), s) == 0.01)
    medium_perfect = grade_medium(1.0, st(diagnosed=True,diagnosed_service="redis",
        diagnosed_cause="memory_leak",remediated=True,remediation_applied="restart_redis"), s)
    chk("medium perfect", 0.55 <= medium_perfect < 1.0, f"score={medium_perfect}")
    ...
    pm = {
        ...
        "timeline_steps":["t0","t1","t2","t3","t4","t5"],
        "prevention_steps":["config_validation_ci","canary_deploy","staging_parity"],
    }
    hard_perfect = grade_hard(1.0, pm, st(diagnosed=True,diagnosed_service="config-service"), s)
    chk("hard perfect", 0.70 <= hard_perfect < 1.0, f"score={hard_perfect}")
    chk("hard zero", grade_hard(0.0, {}, st(), s) == 0.01)

    chk("mttr fast", compute_mttr_bonus(1.0, 1, 15) > 0.9)
    chk("mttr in range", all(0.0 < compute_mttr_bonus(b,s,15) < 1.0 for b in [0,0.5,1] for s in [1,7,15]))
    ...
    for d in ("easy","medium","hard"):
        ...
        chk(
            f"gen_{d}",
            sc["difficulty"]==d
            and sc["correct_playbook"] in sc["available_playbooks"]
            and sc["fault_service"] in sc["services"]
            and "service_logs" in sc
            and "playbook_effects" in sc,
        )
